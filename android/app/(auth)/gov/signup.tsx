...
import useGovtSignup from '@/hooks/useGovtSignup';
...
const Signup = () => {
	...
	const { loading, signup } = useGovtSignup();
					...
					<View className='flex flex-col glassmorphic p-5 gap-6 rounded-lg w-full items-center'>
						...
						<View className="gap-2 w-full">
							<View className="flex flex-row items-center gap-2">
								<Icon2 name="landmark" size={20} color="#D1D5DB" />
								<Text className="text-light-100 text-lg">Government ID</Text>
							</View>
							<TextInput
								...
								value={inputs.govtId}
								onChangeText={text => handleInputChange('govtId', text)}
							/>
						</View>
						...
						<View className="gap-2 w-full">
							...
							<TextInput
								className="input-primary px-6 py-4"
								placeholder="Enter your Government body name"
								placeholderTextColor="#6B7280"
								value={inputs.name}
