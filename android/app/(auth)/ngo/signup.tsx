...
import useNGOSignup from '@/hooks/useNGOSignup';
...
const Signup = () => {
	...
	const { loading, signup } = useNGOSignup();
	...
	const handleInputChange = (key: keyof typeof inputs, value: string) => {
		if (key === "SDG") {
			setInputs(prev => {
				const exists = prev.SDG.includes(value);
				return {
					...prev,
					SDG: exists ? prev.SDG.filter(v => v !== value) : [...prev.SDG, value]
				};
			});
		} else {
					...
					<View className='flex flex-col glassmorphic p-5 gap-6 rounded-lg w-full items-center'>
						...
						<View className="gap-2 w-full">
							<View className="flex flex-row items-center gap-2">
								<Icon2 name="id-badge" size={20} color="#D1D5DB" />
								<Text className="text-light-100 text-lg">NGO Registration ID</Text>
							</View>
							<TextInput
								...
								value={inputs.regId}
								onChangeText={text => handleInputChange('regId', text)}
							/>
						</View>
						...
						<View className="gap-2 w-full">
							<View className="flex flex-row items-center gap-2">
								<Icon2 name="bullseye" size={20} color="#D1D5DB" />
								<Text className="text-light-100 text-lg">Aim</Text>
							</View>
							<TextInput
								className="input-primary px-6 py-4"
								placeholder="Enter the aim of your NGO"
								placeholderTextColor="#6B7280"
								value={inputs.aim}
								onChangeText={text => handleInputChange('aim', text)}
							/>
						</View>

						<View className="gap-2 w-full">
							<View className="flex flex-row items-center gap-2">
								<Icon name="globe" size={20} color="#D1D5DB" />
								<Text className="text-light-100 text-lg">SDGs</Text>
							</View>
							<View className="flex flex-row px-2 py-2 justify-between">
								{["SDG 13", "SDG 14", "SDG 15"].map((g) => {
									const num = g.split(" ")[1];
									const selected = inputs.SDG.includes(num);

									return (
										<TouchableOpacity
											key={g}
											className="flex flex-row items-center gap-2"
											onPress={() => handleInputChange("SDG", num)}
										>
											<View className="w-5 h-5 rounded-full border-2 border-gray-400 items-center justify-center">
												{selected && <View className="w-3 h-3 rounded-full bg-gray-300" />}
											</View>
