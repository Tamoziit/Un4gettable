import type { OpenCommunities } from "../../types";
...
import useJoinOpenCommunity from "../../hooks/useJoinOpenCommunity";
import Spinner from "../Spinner";
import { Link } from "react-router-dom";

interface OpenCommunityProps {
	community: OpenCommunities;
	role: "display" | "join"
}

const OpenCommunityCard = ({ community, role }: OpenCommunityProps) => {
	const { loading, joinOpenCommunity } = useJoinOpenCommunity();
		...
		>
			<div className="h-50 bg-gradient-to-br from-green-400 to-emerald-600 relative">
				...
				<div className="absolute bottom-4 left-4">
					<span className={`bg-gray-200 bg-opacity-90 ${communityColor[community.SDG]} px-3 py-1 rounded-full text-sm md:text-base font-semibold`}>
						{community.SDG}
					</span>
				</div>
			...
			<div className="p-4">
				...
				{role === "join" && (
					<button
						className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white font-semibold py-2 px-4 rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all duration-200 flex items-center justify-center gap-2 group-hover:gap-3 cursor-pointer"
						disabled={loading}
						onClick={() => joinOpenCommunity(community._id)}
					>
						{loading ? (
							<Spinner size="small" />
						) : (
				...
				{role === "display" && (
					<Link
						to={`/open-community/${community._id}`}
...
export default OpenCommunityCard;